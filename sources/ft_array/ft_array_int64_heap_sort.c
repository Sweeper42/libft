...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_array_int64_heap_sort.c                         :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: nperrin <nperrin@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/11/13 14:31:19 by nperrin           #+#    #+#             */
/*   Updated: 2017/11/13 14:37:17 by nperrin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
							...
							size_t end)
{
	...
	while ((child = LCHILD___(start)) <= end)
	{
		...
		if ((child + 1 <= end) && (array[higher] < array[child + 1]))
			higher = child + 1;
		if (higher != start)
			ft_swap_int64(array + start, array + higher);
		else
			break ;
		start = higher;
	}
							...
							size_t n)
{
	...
	while (i < n - 1)
	{
		ft_swap_int64(array + n - i - 1, array + 0);
		i++;
