...
/*   Updated: 2017/11/07 14:56:45 by nperrin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
					...
					t_lext_action_table *r_action_table)
{
	...
	while (cur_state < r_action_table->n_state - 1)
	{
		...
		while (i < r_action_table->n_state)
		{
			...
			{
				if (r_action_table->table[cur_state][j] == r_action_table->table[i][j])
				{
					...
					t_lext_action_table *r_action_table)
{
	...
	while (input_id < r_action_table->n_input)
	{
		...
		{
			printf("stateid: %u value: %u\n", state_id, r_action_table->table[state_id][input_id]);
			count_array[r_action_table->table[state_id][input_id]]++;
			state_id++;
		}
		r_input_point[input_id] = find_bigest_occurence___(count_array, r_action_table->n_state);
		input_id++;
	}
}
					...
					uint16_t size)
{
	...
	while (i < size)
	{
		printf("input[%u] state_id:%u n: %u\n", i, r_input_point[i].most_used_state, r_input_point[i].n);
		i++;
	}
}
					...
					t_error_c **rrc_error)
{
	...
	if (!state_point)
		return (-1);
	input_point = ft_memcalloc(r_action_table->n_input, sizeof(t_input_data), rrc_error);
	if (!input_point)
	{
		free(state_point);
		return (-1);
